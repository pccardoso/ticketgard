    ...
    <div class="p-4 rounded-2xl mb-4 shadow-lg border-1 border-gray-200">
        ...
        <form @submit.prevent="cadastrar()">
            ...
            <div class="grid gap-6 mb-6 md:grid-cols-3">
                <div>
                    <label for="first_name" class="block mb-2 text-sm font-medium text-gray-900">Assunto do
                        Ticket:</label>
                    <input v-model="form_temp.assunto_chamados" type="text" id="first_name"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                        placeholder="Nome do departamento..." />
                </div>

                <div>
                    <label for="first_name" class="block mb-2 text-sm font-medium text-gray-900">Departamento
                        Responsável:</label>
                    <select @change="selectSolicitacao()" v-model="form_temp.id_departamento_chamados"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                        <option value="0">- selecione o departamento -</option>
                        <option v-for="(l, index) in listaDepartameto" :key="index" :value="l.id_departamentos">{{
                            l.nome_departamentos }}</option>
                    </select>
                </div>

                <div>
                    <label for="first_name" class="block mb-2 text-sm font-medium text-gray-900">Tipo de
                        Solicitação:</label>
                    <select id="sSelSol" v-model="form_temp.id_solicitacao_chamados"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                        <option value="0">- selecione o tipo de solicitação -</option>
                        <option v-for="(l, index) in listaSolicitacao" :key="index" :value="l.id_solicitacoes">{{
            ...
            <div class="mt-2 mb-2">
                <label for="first_name" class="block mb-2 text-sm font-medium text-gray-900">Descrição do
                    Chamado:</label>
            ...
            <div class="bg-blue-100 border border-blue-300 text-blue-800 px-4 py-3 rounded-lg mb-4 flex items-center my-5"
                role="alert">
                <span>A <strong>edição do ticket</strong> está disponível <strong>apenas enquanto estiver
                        aberto</strong>. Depois de atribuído a um atendente, não será mais possível alterar as
                    informações.</span>
            </div>
            ...
            <div class="grid gap-6 mb-6 md:grid-cols-3">
                ...
                <div>
                    <input type="file" hidden multiple accept=".jpg,.png,.jpeg,.pdf,.xls" @change="handleFileChange"
                        id="file"
                        class="bg-gray-50 border border-gray-300 text-blue-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full" />
                    <label for="file"
                        class="inline-flex bg-gray-200 p-3 rounded-3xl hover:shadow-sm hover:bg-blue-500 hover:text-white mt-3">

                        <svg class="w-5 h-5 mr-2 text-gray-800" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
            ...
            <div v-if="form_temp.file.length" class="p-3 bg-gray-100 rounded-2xl w-110 shadow-inner">
                ...
                <div v-for="(f, index) in form_temp.file" :key="index"
                    ...
                    <div class="flex items-center gap-2">
                        <svg class="w-5 h-5 text-blue-500" xmlns="http://www.w3.org/2000/svg" fill="currentColor"
                            viewBox="0 0 24 24">
                            <path fill-rule="evenodd"
                    ...
                    <div @click="excluirAnexo(index)"
                        class="p-1 rounded-full hover:bg-red-100 cursor-pointer transition-colors">
